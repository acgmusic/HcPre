#!/bin/bash
# Verify mhc_pre_sinkhorn debug build: ini -> gen script -> .o debug sections -> source refs
set -u
...
for f in $REPO/build/binary/ascend910_93/gen/MhcPreSinkhorn-*.sh; do
  grep -o "op_debug_config=[a-z_0-9;]*" "$f" | head -1 && break
done

echo
echo "=== [3] kernel .o debug sections ==="
for OBJ in $(find $REPO/build/binary/ascend910_93/bin/mhc_pre_sinkhorn -name "*.o" | head -4); do
...
OBJ=$(find $REPO/build/binary/ascend910_93/bin/mhc_pre_sinkhorn -name "*.o" | head -1)
strings -a "$OBJ" | grep -E "mhc_pre_sinkhorn.*\.(cpp|h)$" | sort -u | head -6
'
