                                                                              ...
                                                                              uint64_t dims, const char *shapeName)
{
    ...
    OP_TILING_CHECK(
        (dimNum != dims),
        OP_LOGE_FOR_INVALID_SHAPEDIM(opName_, shapeName, std::to_string(dimNum) + "D", std::to_string(dims) + "D"),
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckOpInputSingleParamsTensorNotSupport()
{
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(opName_, "sendCountsTensor, recvCountsTensor", "not null",
    ...
    OP_TILING_CHECK(commQuantScaleTensorShape != nullptr,
                    OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "commQuantScale", "not null",
                                                          "The value of commQuantScale must be nullptr."),
                    return ge::GRAPH_FAILED);
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckOpInputSingleParamsTensorSupport()
{
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(opName_, "gmmXScale, gmmWeightScale", "null",
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckOpInputSingleParamsTensorMM()
{
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(
            opName_, "mmXTensor, mmWeightTensor, mmYTensor",
            (std::string(isMmXNull ? "null" : "not null") + ", " + std::string(isMmWeightNull ? "null" : "not null") +
             ", " + std::string(isMmYNull ? "null" : "not null"))
                .c_str(),
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckAndSetLocalParamsMm()
{
    ...
    OP_TILING_CHECK(
        !IsContains(QUANT_GMM_X_DTYPE_LIST, localParams_.mmXDtype),
        OP_LOGE_FOR_INVALID_DTYPE(opName_, "mmX", Ops::Base::ToString(localParams_.mmXDtype).c_str(), "(DT_HIFLOAT8)"),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(!IsContains(QUANT_GMM_WEIGHT_DTYPE_LIST, localParams_.mmWeightDtype),
                    OP_LOGE_FOR_INVALID_DTYPE(opName_, "mmWeight",
                                              Ops::Base::ToString(localParams_.mmWeightDtype).c_str(), "(DT_HIFLOAT8)"),
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(localParams_.Bs != mmYDim0,
                    OP_LOGE_FOR_INVALID_SHAPE(opName_, "mmX, mmY",
                                              (std::string("mmX.DIM0=") + std::to_string(localParams_.Bs) +
                                               ", mmY.DIM0=" + std::to_string(mmYDim0))
                                                  .c_str(),
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckAndSetLocalParamsAttr()
{
    ...
    OP_TILING_CHECK(gmmXQuantModeptr == nullptr,
                    OP_LOGE_WITH_INVALID_ATTR(opName_, "gmmXQuantMode", "null", "not null"), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(gmmWeightQuantModeptr == nullptr,
                    OP_LOGE_WITH_INVALID_ATTR(opName_, "gmmWeightQuantMode", "null", "not null"),
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(gmmTransWeightptr == nullptr,
                    OP_LOGE_WITH_INVALID_ATTR(opName_, "transGmmWeight", "null", "not null"), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(commQuantModeptr == nullptr,
                    OP_LOGE_WITH_INVALID_ATTR(opName_, "commQuantMode", "null", "not null"), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(commQuantDtypeptr == nullptr,
                    OP_LOGE_WITH_INVALID_ATTR(opName_, "commQuantDtype", "null", "not null"), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(localParams_.commQuantMode != QUANT_NONE,
                    OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "commQuantMode",
                                                          std::to_string(localParams_.commQuantMode).c_str(),
                                                          "commQuant is not supported yet; only 0 is allowed"),
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(mmWeightQuantModeptr == nullptr,
                    OP_LOGE_WITH_INVALID_ATTR(opName_, "mmWeightQuantMode", "null", "not null"),
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(mmTransWeightptr == nullptr,
                    OP_LOGE_WITH_INVALID_ATTR(opName_, "transMmWeight", "null", "not null"), return ge::GRAPH_FAILED);
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckFormat()
{
    ...
    OP_TILING_CHECK(
        context_->GetInputDesc(GMM_X_INDEX)->GetStorageFormat() != ge::Format::FORMAT_ND,
    ...
    OP_TILING_CHECK(context_->GetInputDesc(GMM_WEIGHT_INDEX)->GetStorageFormat() != ge::Format::FORMAT_ND,
                    ...
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        context_->GetRequiredInputDesc(GMM_X_SCALE_INDEX)->GetStorageFormat() != ge::Format::FORMAT_ND,
        OP_LOGE_FOR_INVALID_FORMAT(
            opName_, "gmmXScale",
        ...
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        context_->GetRequiredInputDesc(GMM_WEIGHT_SCALE_INDEX)->GetStorageFormat() != ge::Format::FORMAT_ND,
        OP_LOGE_FOR_INVALID_FORMAT(
            opName_, "gmmWeightScale",
    ...
    if (mmXDesc != nullptr) {
        OP_TILING_CHECK(mmXDesc->GetStorageFormat() != ge::Format::FORMAT_ND,
                        OP_LOGE_FOR_INVALID_FORMAT(
                            opName_, "mmX", Ops::Base::ToString(mmXDesc->GetStorageFormat()).c_str(), "FORMAT_ND"),
                        return ge::GRAPH_FAILED);
    }
    ...
    if (mmYDesc != nullptr) {
        OP_TILING_CHECK(mmYDesc->GetStorageFormat() != ge::Format::FORMAT_ND,
                        OP_LOGE_FOR_INVALID_FORMAT(
                            opName_, "mmY", Ops::Base::ToString(mmYDesc->GetStorageFormat()).c_str(), "FORMAT_ND"),
                        return ge::GRAPH_FAILED);
    }
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckParamsRelationGmm()
{
    ...
    OP_TILING_CHECK(!IsContains(QUANT_GMM_X_SCALE_DTYPE_LIST, localParams_.gmmXScaleDtype),
                    OP_LOGE_FOR_INVALID_DTYPE(opName_, "gmmXScale",
                                              Ops::Base::ToString(localParams_.gmmXScaleDtype).c_str(), "(DT_FLOAT)"),
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(gmmXScaleDim0 != 1,
                    OP_LOGE_FOR_INVALID_SHAPE(opName_, "gmmXScale", "[" + std::to_string(gmmXScaleDim0) + "]", "[1]"),
                    return ge::GRAPH_FAILED);
    int64_t gmmWeightScaleDim0 = gmmWeightScaleStorageShape->GetStorageShape().GetDim(DIM_ZERO);
    OP_TILING_CHECK(
        gmmWeightScaleDim0 != 1,
        OP_LOGE_FOR_INVALID_SHAPE(opName_, "gmmWeightScale", "[" + std::to_string(gmmWeightScaleDim0) + "]", "[1]"),
        return ge::GRAPH_FAILED);
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckParamsRelationMm()
{
    ...
    OP_TILING_CHECK(!IsContains(QUANT_GMM_X_SCALE_DTYPE_LIST, localParams_.mmXScaleDtype),
                    OP_LOGE_FOR_INVALID_DTYPE(opName_, "mmXScale",
                                              Ops::Base::ToString(localParams_.mmXScaleDtype).c_str(), "(DT_FLOAT)"),
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(localParams_.mmXQuantMode != QUANT_PERTENSOR,
                    OP_LOGE_WITH_INVALID_ATTR(opName_, "mmXQuantMode", std::to_string(localParams_.mmXQuantMode),
                                              std::to_string(QUANT_PERTENSOR) + " (PERTENSOR)"),
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(mmXScaleDim0 != 1,
                    OP_LOGE_FOR_INVALID_SHAPE(opName_, "mmXScale", "[" + std::to_string(mmXScaleDim0) + "]", "[1]"),
                    return ge::GRAPH_FAILED);
    int64_t mmWeightScaleDim0 = mmWeightScaleStorageShape->GetStorageShape().GetDim(DIM_ZERO);
    OP_TILING_CHECK(
        mmWeightScaleDim0 != 1,
        OP_LOGE_FOR_INVALID_SHAPE(opName_, "mmWeightScale", "[" + std::to_string(mmWeightScaleDim0) + "]", "[1]"),
        return ge::GRAPH_FAILED);
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckParamsAttrEpAndSetLocalParams()
{
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE_WITH_INVALID_ATTR(opName_, "epWorldSize", std::to_string(rankDim), supportRankSizeRange.c_str()),
        return ge::GRAPH_FAILED);
    ...
    if (isEpNumMatch) {
        ...
        OP_TILING_CHECK(expertNum > MAX_EXPERT_NUM,
                        OP_LOGE_FOR_INVALID_VALUE(opName_, "expertNum", std::to_string(expertNum),
                                                  "<= " + std::to_string(MAX_EXPERT_NUM)),
                        return ge::GRAPH_FAILED);
    } else {
        OP_LOGE_FOR_INVALID_VALUE(opName_, "ep", std::to_string(localParams_.ep),
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "groupSize", std::to_string(localParams_.groupSize),
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckAndSetSendRecvCountsAttr()
{
    ...
    OP_TILING_CHECK(sendCountsSize != recvCountsSize,
                    OP_LOGE_FOR_INVALID_LISTSIZE(
                        opName_, "sendCounts, recvCounts",
                        std::to_string(sendCountsSize) + " vs " + std::to_string(recvCountsSize), "must be equal"),
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        sendCountsSize != expertNum,
        OP_LOGE_FOR_INVALID_LISTSIZE(opName_, "sendCounts", std::to_string(sendCountsSize), std::to_string(expertNum)),
        return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(sendCountsSum != localParams_.A,
                    OP_LOGE_FOR_INVALID_VALUE(opName_, "sendCountsSum", std::to_string(sendCountsSum),
    ...
    OP_TILING_CHECK(recvCountsSum != localParams_.BsK,
                    OP_LOGE_FOR_INVALID_VALUE(opName_, "recvCountsSum", std::to_string(recvCountsSum),
    ...
    for (uint64_t i = 0; i < maxCountsSize; i++) {
        OP_TILING_CHECK(
            ...
            OP_LOGE_FOR_INVALID_VALUE(opName_, (std::string("sendCounts[") + std::to_string(i) + "]").c_str(),
                                      std::to_string(sendCounts[i]), "[0, " + std::to_string(localParams_.A) + "]"),
            return ge::GRAPH_FAILED);
        OP_TILING_CHECK(
            recvCounts[i] < 0 || recvCounts[i] > static_cast<int64_t>(localParams_.BsK),
            OP_LOGE_FOR_INVALID_VALUE(opName_, (std::string("recvCounts[") + std::to_string(i) + "]").c_str(),
                                      std::to_string(recvCounts[i]), "[0, " + std::to_string(localParams_.BsK) + "]"),
            return ge::GRAPH_FAILED);
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::CheckLocalParams()
{
    OP_TILING_CHECK((localParams_.H1 == 0) || (localParams_.H1 >= MAX_H1_VALUE),
                    OP_LOGE_FOR_INVALID_VALUE(opName_, "H1", std::to_string(localParams_.H1),
                                              "(0, " + std::to_string(MAX_H1_VALUE) + ")"),
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK((localParams_.BsK == 0) || (localParams_.BsK >= MAX_BSK_VALUE),
                    OP_LOGE_FOR_INVALID_VALUE(opName_, "BsK", std::to_string(localParams_.BsK),
                                              "(0, " + std::to_string(MAX_BSK_VALUE) + ")"),
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK((localParams_.N1 == 0) || (localParams_.N1 >= MAX_N1_VALUE),
                    OP_LOGE_FOR_INVALID_VALUE(opName_, "N1", std::to_string(localParams_.N1),
    ...
    OP_TILING_CHECK(
        (localParams_.Bs == 0) || (localParams_.BsK % localParams_.Bs != 0),
    ...
    OP_TILING_CHECK((localParams_.N2 == 0) || (localParams_.N2 >= MAX_N2_VALUE),
                    OP_LOGE_FOR_INVALID_VALUE(opName_, "N2", std::to_string(localParams_.N2),
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::SetTilingCommonInfo()
{
    ...
    expertNum_ = static_cast<uint32_t>(Ops::Base::CeilDiv(localParams_.ep, static_cast<uint64_t>(TARGET_COMM_ROUNDS)));
    if (expertNum_ == 0U) {
        expertNum_ = 1U;
    }
    OP_LOGD(opName_, "SetTilingCommonInfo: ep=%lu, expertNum_=%u, targetCommRounds=%u", localParams_.ep, expertNum_,
            TARGET_COMM_ROUNDS);
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::DoQuantGMMTiling()
{
    ...
    OP_TILING_CHECK(
        expertNum_ == 0,
        OP_LOGE_FOR_INVALID_VALUE(opName_, "expertNum_", std::to_string(expertNum_).c_str(), "positive value"),
        return ge::GRAPH_FAILED);
    expertNum_ = Ops::Base::CeilDiv(localParams_.ep, expertNum_) > MAX_HCCL_HANDLE_NUM ?
                     Ops::Base::CeilDiv(localParams_.ep, static_cast<uint64_t>(MAX_HCCL_HANDLE_NUM)) :
                     expertNum_;
    MC2_CHECK_LOG_RET(opName_, gmmTile.SetGroupExpertInputParameters(localParams_, mMaxSize, expertNum_));
...
ge::graphStatus QuantGroupedMatmulAllToAllvTilingCommon::SetHcclTiling()
{
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(opName_, "outputDataType", Ops::Base::ToString(outputDataType).c_str(),
                                              "The dtype of outputDataType must be within the supported range."),
        return ge::GRAPH_FAILED);
