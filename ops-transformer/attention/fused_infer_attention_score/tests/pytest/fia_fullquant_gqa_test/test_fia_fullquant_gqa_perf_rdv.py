...
import fia_fullquant_gqa_paramset_perf_rdv as paramset
...
def _build_parametrize_args():
    ...
    for case in CASES:
        if case["name"] in SKIP_CASES:
            args.append(
                pytest.param(
                    case, marks=pytest.mark.skip(reason="execution time too long")
                )
            )
        else:
            args.append(case)
...
def test_fia_fullquant_gqa(params, golden_mode, cache_dir, load_pt_dir):
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(
            test_runner.execute_test, params, golden_mode, cache_dir, load_pt_dir
        )
