...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        default=False,
        help=(
            "gen 阶段使用外部 NPU 排布 pt 文件作为输入（替代 generate_data）。"
            "需配合 --load-pt-dir 指定目录。"
        ),
    )
    parser.addoption(
        "--load-pt-dir",
        default=None,
        metavar="PT_DIR",
        help="外部 NPU 排布 pt 文件所在目录（--use-external-input 时生效）",
    )
    parser.addoption(
        "--parse-prof",
        default=None,
...
@pytest.fixture(scope="session")
def load_pt_dir(request):
    """返回 (use_external, pt_dir) 或 None"""
    use_external = request.config.getoption("--use-external-input", default=False)
    pt_dir = request.config.getoption("--load-pt-dir", default=None)
    if use_external and not pt_dir:
        raise pytest.UsageError("--use-external-input requires --load-pt-dir")
    return pt_dir if use_external else None
