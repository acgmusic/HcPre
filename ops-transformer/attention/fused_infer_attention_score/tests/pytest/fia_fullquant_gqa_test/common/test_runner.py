...
from common import fia_fullquant_gqa_golden as golden
from common import golden_cache
from common import result_compare_method
from common import load_external_data
...
def execute_test(params, mode, cdir=None, external_dir=None):
    """执行单个 case 的全流程

    mode: {"gen","cpu","npu","compare"} 的子集
    cdir: golden 缓存目录
    external_dir: 若指定且 mode 含 gen，则从该目录加载外部 NPU 排布数据
    ...
    use_external = external_dir is not None

    # ---- Step 1: 输入数据 ----
    if "gen" in mode:
        if use_external:
            ext = load_external_data.load_data_from_dir(external_dir)
            golden.B = ext.B
            golden.N_q = ext.N_q
            golden.N_kv = ext.N_kv
            golden.D = ext.D
            golden.BLOCK_SIZE = ext.block_size
            golden.ACTUAL_SEQ_Q = ext.ACTUAL_SEQ_Q
            golden.ACTUAL_SEQ_KV = ext.ACTUAL_SEQ_KV
            bnsd_tup = golden.external_to_bnsd(ext)
            block_table_torch = ext.block_table
        else:
    ...
    if "cpu" in mode:
        if golden.NUM_BLOCKS != 0:
            ...
            cpu_out, cpu_lse = golden.cpu_fp8_fullquant_gqa_golden(
                q_fp8,
                k_fp8,
                v_fp8,
                deq_q,
    ...
    cache_info = None
    if "npu" in mode:
        with torch.profiler.record_function(f"gqa_fa::{case_name}"):
