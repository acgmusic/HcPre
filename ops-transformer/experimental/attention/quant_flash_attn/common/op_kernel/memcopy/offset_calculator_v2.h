...
template <>
struct GmLayoutParams<GmFormat::NBSGD2> {
    static constexpr FormatCategory CATEGORY = FormatCategory::GM_Q_OUT_NBSGD2;
};
...
template <>
struct GmLayoutParams<GmFormat::BNGSD2> {
    static constexpr FormatCategory CATEGORY = FormatCategory::GM_Q_OUT_BNGSD;
};
...
struct OffsetCalculatorImpl<FORMAT, FormatCategory::GM_Q_OUT_NBSGD2, ACTLEN_T, WITH_ZERO_HEAD> {
    GmLayout<FORMAT> gmLayout;
    ActualSeqLensParser<ActualSeqLensMode::BY_BATCH, ACTLEN_T> actualSeqLensQParser;

    __aicore__ inline OffsetCalculatorImpl() = default;

    __aicore__ inline void Init(uint32_t n2, uint32_t b, uint32_t g, uint32_t s, uint32_t d,
                                const ActualSeqLensParser<ActualSeqLensMode::BY_BATCH, ACTLEN_T> &seqLensParser)
    {
        this->actualSeqLensQParser = seqLensParser;
        gmLayout.MakeLayout(n2, b, s, g, d);
    }

    __aicore__ inline uint64_t GetOffset(uint32_t bIdx, uint32_t n2Idx, uint32_t gIdx, uint32_t s1Idx, uint32_t dIdx)
    {
        uint64_t offset = n2Idx * GetStrideN2() + bIdx * GetStrideB() + s1Idx * GetStrideS1() + gIdx * GetStrideG() +
                          dIdx * GetStrideD();
        return offset;
    }
...
struct OffsetCalculatorImpl<FORMAT, FormatCategory::GM_Q_OUT_TND, ACTLEN_T, WITH_ZERO_HEAD> {
    ...
    Init(uint32_t n2, uint32_t g, uint32_t d,
...
struct OffsetCalculatorImpl<FORMAT, FormatCategory::GM_KV_PA_BNBD, ACTLEN_T> {
    ...
    __aicore__ inline void Init(uint32_t n2, uint32_t blockSize, uint32_t d, __gm__ uint8_t *blockTableGmAddr,
...
struct OffsetCalculatorImpl<FORMAT, FormatCategory::GM_ANTIQ_BnNBs, ACTLEN_T> {
    ...
    __aicore__ inline void Init(uint32_t n, uint32_t blockSize, __gm__ uint8_t *blockTableGmAddr,
                                uint32_t maxblockNumPerBatch)
    {
        blockTableParser.Init(blockTableGmAddr, maxblockNumPerBatch);
        gmLayout.MakeLayout(n, blockSize);
