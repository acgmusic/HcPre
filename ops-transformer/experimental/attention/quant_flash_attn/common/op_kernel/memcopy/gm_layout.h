...
template <>
struct GmLayout<GmFormat::BNGSD2> {
    AscendC::Shape<uint32_t, uint32_t, uint32_t, uint32_t, uint32_t> shape;
    AscendC::Stride<uint64_t, uint64_t, uint64_t, uint64_t, uint64_t> stride;
...
template <>
struct GmLayout<GmFormat::NBSGD2> {
    AscendC::Shape<uint32_t, uint32_t, uint32_t, uint32_t, uint32_t> shape;
    AscendC::Stride<uint64_t, uint64_t, uint64_t, uint64_t, uint64_t> stride;

    __aicore__ inline GmLayout() = default;
    __aicore__ inline void MakeLayout(uint32_t n2, uint32_t b, uint32_t s, uint32_t g, uint32_t d)
    {
        shape = AscendC::MakeShape(n2, b, s, g, d);
        uint64_t dStride = 1;
        uint64_t gStride = dStride * d;
        uint64_t sStride = gStride * g;
        uint64_t bStride = sStride * s;
        uint64_t n2Stride = bStride * b;
        stride = AscendC::MakeStride(n2Stride, bStride, sStride, gStride, dStride);
    }
};
