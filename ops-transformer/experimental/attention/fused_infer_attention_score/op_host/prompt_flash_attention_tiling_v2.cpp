...
ge::graphStatus PromptFlashAttentionTilingV2::CheckEmptyTensor(ContextParamsForPFATiling &contextKeyParams)
{
    OP_CHECK_IF((contextKeyParams.queryInputShape->GetStorageShape().GetShapeSize() == 0 &&
                ...
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "query shape size is %llu byte, but attention Out shape size is %llu byte, "
                                            "they cannot be empty while the other is not",
...
bool PromptFlashAttentionTilingV2::CheckIODataType(ContextParamsForPFATiling &contextKeyParams)
{
    ...
    std::vector<ge::DataType> allowedDtypes = {ge::DT_FLOAT16, ge::DT_BF16, ge::DT_INT8, ge::DT_HIFLOAT8,
                                               ge::DT_FLOAT8_E4M3FN};
    ...
    OP_CHECK_IF((inputTypeCheck == allowedDtypes.end() || outputTypeCheck == allowedDtypes.end()),
                OPS_REPORT_VECTOR_INNER_ERR(
                    contextKeyParams.opName, "inputType(%s) and outputType(%s) should be DT_FLOAT16, DT_BF16, DT_INT8.",
...
bool PromptFlashAttentionTilingV2::SetShape(ContextParamsForPFATiling &contextKeyParams,
                                            const gert::StorageShape *shape, const std::string inputName, int64_t &b,
                                            int64_t &n, int64_t &s, int64_t &d, int64_t &h, int64_t &t) const
{
                                                    ...
                                                    const std::string &sName) const
{
    ...
    OP_CHECK_IF(
        (shape->GetStorageShape().GetDimNum() != 3) &&
        ...
        return false);
    OP_CHECK_IF(
        (shape->GetStorageShape().GetDimNum() != 4) &&
            (inputLayout != InputLayout::BSH && inputLayout != InputLayout::TND && inputLayout != InputLayout::NTD),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
    ...
    OP_CHECK_IF((b > BLIMIT || b <= 0),
                OPS_REPORT_VECTOR_INNER_ERR(
                    ...
                    "batch size of %s should be less than or equal to %u and > 0, but batch size = %ld.", sName.c_str(),
                    BLIMIT, b),
                return false);
    OP_CHECK_IF((s < 0),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "seq size of %s should >= 0, but sequence size = %ld.", sName.c_str(), s),
                return false);
    if (s > SLIMIT) {
    ...
    if (inputLayout == InputLayout::BSH) {
        OP_CHECK_IF((d > DLIMIT || d <= 0),
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "d size of %s should be less than or equal to %u and > 0, but d = %ld, "
                                                "and layout is BSH, d = h / headsNumber.",
    ...
    OP_CHECK_IF(
        (inputLayout == InputLayout::TND) && (t <= 0),
...
void PromptFlashAttentionTilingV2::GetQueryDimAndOutDim(const gert::StorageShape *queryShape,
                                                        ...
                                                        const std::string &layoutStr, int64_t &tmpqueryDim,
                                                        int64_t &outDim, uint32_t i) const
{
                                                                  ...
                                                                  const gert::StorageShape *outShape) const
{
    ...
    OP_CHECK_IF((queryDimNum < INPUT_QKV_SHAPE_MIN_DIMS) || (queryDimNum > INPUT_QKV_SHAPE_MAX_DIMS),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "tensor query shape dimNum(%zu) is invalid! Only support range [%u, %u].",
                                            queryDimNum, INPUT_QKV_SHAPE_MIN_DIMS, INPUT_QKV_SHAPE_MAX_DIMS),
                return false);
    ...
    for (uint32_t i = 0; i < queryDimNum; ++i) {
        ...
        OP_CHECK_IF(!isQKVDDifferent && (tmpqueryDim != outDim),
                    OPS_REPORT_VECTOR_INNER_ERR(
                        contextKeyParams.opName,
                        "tensor query shape (%ld) does not equal to tensor output shape(%ld) in dim %u for %s.",
...
bool PromptFlashAttentionTilingV2::CheckRopeDataType(ContextParamsForPFATiling &contextKeyParams) const
{
    ...
    OP_CHECK_IF((queryDataType != queryRopeDataType),
                OPS_REPORT_VECTOR_INNER_ERR(
                    contextKeyParams.opName, "DataType of query rope(%s) is not equal to datatype of query(%s).",
                    GetPfaDataTypeStr(queryRopeDataType).c_str(), GetPfaDataTypeStr(queryDataType).c_str()),
                return false);
    OP_CHECK_IF((keyDataType != keyRopeDataType),
                OPS_REPORT_VECTOR_INNER_ERR(
                    contextKeyParams.opName, "DataType of key rope(%s) is not equal to datatype of key(%s).",
                                                                  ...
                                                                  const gert::StorageShape *valueShape)
{
    ...
    for (uint32_t i = 0; i < keyDimNum; ++i) {
        ...
        OP_CHECK_IF(
            !isQKVDDifferent && (tmpKeyDim != tmpValueDim),
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "tensor key shape(%ld) does not equal to tensor value shape(%ld) in dim %u.",
                                        tmpKeyDim, tmpValueDim, i),
            return false);
        OP_CHECK_IF(
            isQKVDDifferent && (i != keyDimNum - 1) && (tmpKeyDim != tmpValueDim),
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "tensor key shape(%ld) does not equal to tensor value shape(%ld) in dim %u.",
                                        tmpKeyDim, tmpValueDim, i),
            return false);
    ...
    if (enableKVAntiquant || enablePerblockQuant || enablePertensorQuant) {
        ...
        if (nQ / nKV > GLIMIT_64) { // G cannot be greater than 64.
            OP_LOGE(contextKeyParams.opName,
                    "In antiquant and fullquant scenario, the G(numHeads / numKeyValueHeads) cannot be larger than 64, "
                    "but G = %d",
                    nQ / nKV);
            return false;
        }
        ...
        if ((enableIFAMLA || enableIFAMLAFullQuant) && (nQ / nKV > GLIMIT_128)) { // G cannot be greater than 128.
            OP_LOGE(contextKeyParams.opName,
                    "In mla decode (non quant and fullquant) scenario, the G(numHeads / numKeyValueHeads) cannot be "
                    "larger than 128, but G = %d",
                    nQ / nKV);
            return false;
        }
    }
                                                             ...
                                                             const PFAShapeInfo &queryShapeInfo) const
{
    ...
    OP_CHECK_IF((contextKeyParams.queryQuantMode == nullptr || contextKeyParams.keyAntiquantMode == nullptr ||
                ...
                return false);
    OP_CHECK_IF(
        (*contextKeyParams.queryQuantMode != static_cast<int64_t>(AntiquantTypeEnum::PER_CHANNEL) ||
        ...
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    ...
                                    *contextKeyParams.queryQuantMode, *contextKeyParams.keyAntiquantMode,
                                    *contextKeyParams.valueAntiquantMode),
        return false);
    ...
    OP_CHECK_IF(
        ...
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "%s is not supported in per-tensor quant scenario.",
                                    layoutStr.c_str()),
        return false);
    OP_CHECK_IF(
    ...
    OP_CHECK_IF(keyShapeD != valueShapeD,
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "The D size of keyInputShape and valueInputShape must be equal in "
                                            "per-tensor quant scenario, but now keyShapeD is %lu, valueShapeD is %lu.",
                                                            ...
                                                            const PFAShapeInfo &valueShapeInfo) const
{
    ...
    OP_CHECK_IF(
        (dequantScaleQueryShape == nullptr) || (keyAntiquantScaleShape == nullptr) ||
    ...
    OP_CHECK_IF((quantScale1Shape != nullptr) && (quantScale1Shape->GetStorageShape().GetDimNum() != 1),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "quantScale1's dim must be 1 in per-block quant scenario, "
    ...
    OP_CHECK_IF((quantScale1Shape != nullptr) && (quantScale1Shape->GetStorageShape().GetDimNum() == 1) &&
                ...
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "quantScale1's shape must be [1] in per-block quant scenario, "
    ...
    OP_CHECK_IF(
        ...
        return false);
    OP_CHECK_IF(
        (dequantScaleQueryType != ge::DT_FLOAT) || (KeyAntiquantScaleType != ge::DT_FLOAT) ||
        ...
        return false);
    const std::vector<std::string> unsupportedLayoutList = {"BNSD_NBSD", "BSH_NBSD",  "BSH_BNSD",
                                                            "BSND_BNSD", "BSND_NBSD", "NTD"};
    OP_CHECK_IF(
        (std::find(unsupportedLayoutList.begin(), unsupportedLayoutList.end(), layoutStr) !=
    ...
    if (inputLayout == InputLayout::NTD) {
        ...
        if (isMaxWorkspace)
            return true;
        OP_CHECK_IF(
            (dequantScaleQueryShape->GetStorageShape().GetDim(0) != queryShapeInfo.n) ||
            ...
            return false);
        OP_CHECK_IF(
            (keyAntiquantScaleShape->GetStorageShape().GetDim(0) != keyShapeInfo.n) ||
        ...
        OP_CHECK_IF((valueAntiquantScaleshape->GetStorageShape().GetDim(0) != valueShapeInfo.n) ||
                        (!(valueAntiquantScaleshape->GetStorageShape().GetDim(1) ==
                           ...
                           valueAntiquantScaleshape->GetStorageShape().GetDim(1) ==
                               valueShapeInfo.t / optFp8VBlockSize + valueShapeInfo.b)) ||
                        (!(valueAntiquantScaleshape->GetStorageShape().GetDim(2) ==
                               CeilDivision(valueShapeInfo.d, fp8KVBlockSize) ||
                           valueAntiquantScaleshape->GetStorageShape().GetDim(2) ==
                               CeilDivision(valueShapeInfo.d, optFp8VBlockSize))),
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "When layout is %s, valueAntiquantScaleshape must be [%u, %u, %u] or "
                                                "[%u, %u, %u] in per-block quant scenario, now is [%u, %u, %u].",
                                                layoutStr.c_str(), valueShapeInfo.n,
                                                valueShapeInfo.t / fp8KVBlockSize + valueShapeInfo.b,
                                                CeilDivision(valueShapeInfo.d, fp8KVBlockSize), valueShapeInfo.n,
                                                valueShapeInfo.t / optFp8VBlockSize + valueShapeInfo.b,
                                                CeilDivision(valueShapeInfo.d, optFp8VBlockSize),
        ...
        OP_CHECK_IF(
            ...
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "innerPrecise [%d] is currently not supported.(Value 2 or 3 is not supported).",
                                        innerPrecise),
            return false);
    } else {
        OP_CHECK_IF(
            (dequeryDim != 4) || (dekeyDim != 4) ||
            ...
            return false);
        OP_CHECK_IF(
            (dequantScaleQueryShape->GetStorageShape().GetDim(0) != queryShapeInfo.b) ||
            ...
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "dequantScaleQueryShape must be [%u, %u, %u, %u] in per-block quant scenario, "
                                        "now is  [%u, %u, %u, %u].",
            ...
            return false);
        OP_CHECK_IF(
            (keyAntiquantScaleShape->GetStorageShape().GetDim(0) != keyShapeInfo.b) ||
            ...
            return false);
        OP_CHECK_IF(
            (valueAntiquantScaleshape->GetStorageShape().GetDim(0) != valueShapeInfo.b) ||
                (valueAntiquantScaleshape->GetStorageShape().GetDim(1) != valueShapeInfo.n) ||
                (!(valueAntiquantScaleshape->GetStorageShape().GetDim(2) ==
                       CeilDivision(valueShapeInfo.s, static_cast<uint64_t>(fp8KVBlockSize)) ||
                   valueAntiquantScaleshape->GetStorageShape().GetDim(2) ==
                       CeilDivision(
                           valueShapeInfo.s,
                           static_cast<uint64_t>(optFp8VBlockSize)))) ||       // 2 is the dim of dequantscale along s2.
                (valueAntiquantScaleshape->GetStorageShape().GetDim(3) != 1U), // 3 is the dim of dequantscale along d.
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "valueAntiquantScaleshape must be [%u, %u, %u, %u] or [%u, %u, %u, %u] in "
                                        "per-block quant scenario, now is [%u, %u, %u, %u].",
                                        valueShapeInfo.b, valueShapeInfo.n,
                                        CeilDivision(valueShapeInfo.s, static_cast<uint64_t>(fp8KVBlockSize)), 1,
                                        valueShapeInfo.b, valueShapeInfo.n,
                                        CeilDivision(valueShapeInfo.s, static_cast<uint64_t>(optFp8VBlockSize)), 1,
                                        valueAntiquantScaleshape->GetStorageShape().GetDim(0),
                                        valueAntiquantScaleshape->GetStorageShape().GetDim(1),
                                        valueAntiquantScaleshape->GetStorageShape().GetDim(2),
                                                        ...
                                                        const PFAShapeInfo &valueShapeInfo) const
{
    ...
    OP_CHECK_IF((quantOffset2Shape != nullptr) && (quantScale2Type != contextKeyParams.quantOffset2Type),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            ...
                                            GetPfaDataTypeStr(contextKeyParams.quantOffset2Type).c_str()),
                return false);

    OP_CHECK_IF(
        (quantScale2Type != ge::DT_BF16) && (quantScale2Type != ge::DT_FLOAT),
    ...
    if (inputType == ge::DT_BF16) {
        ...
        OP_CHECK_IF(
            quantScale2Type != ge::DT_FLOAT,
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName, "invalid post quant scale dtype(%s), when q is %s, only support float32",
                GetPfaDataTypeStr(quantScale2Type).c_str(), GetPfaDataTypeStr(inputType).c_str()),
            return false);
    }
                                                                  ...
                                                                  const gert::StorageShape *keyShape)
{
    ...
    OP_CHECK_IF((keySharedPrefixDataType != valueSharedPrefixDataType),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When system prefix is used, dataType of keySharedPrefixDataType(%s) and "
                                            "dataType of valueSharedPrefixDataType(%s) must be same.",
                                            GetPfaDataTypeStr(keySharedPrefixDataType).c_str(),
                                            GetPfaDataTypeStr(valueSharedPrefixDataType).c_str()),
                return false);
    return true;
}
                                                        ...
                                                        const uint32_t sPrefix, const uint32_t sKV)
{
    ...
    OP_CHECK_IF((prefixDimNum != 1),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "actualSharedPrefixLen dim num(%zu) should be 1!",
                                            prefixDimNum),
                return false);
    int64_t prefixShapeSize = contextKeyParams.actualSharedPrefixLen->GetStorageShape().GetShapeSize();
    OP_CHECK_IF((prefixShapeSize != 1),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "actualSharedPrefixLen length(%ld) should be 1!",
                                            prefixShapeSize),
                return false);
    ...
    OP_CHECK_IF(
        (actualSharedPrefixLen > sPrefix) || (actualSharedPrefixLen < 0),
                                                        ...
                                                        int64_t headNumRatio)
{
    ...
    OP_CHECK_IF((keyDim == KV_CACHE_DIM_NUMS_5) &&
                ...
                return false);
    OP_CHECK_IF(
        (keyDim == KV_CACHE_DIM_NUMS_4) && ((keyDim1 != valueDim1) || (keyDim2 != valueDim2) ||
    ...
    OP_CHECK_IF((keyDim == KV_CACHE_DIM_NUMS_3) &&
                    ((keyDim1 != valueDim1) || (keyDim2 != valueDim2) || ((keyDim3 != valueDim3) && (!enablePFAMLA))),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "the dim num of key and value are inconsistent when PA enable. key shape "
                                            "[%ld,%ld,%ld], value shape [%ld,%ld,%ld].",
    ...
    if (enableIFAMLAFullQuant) {
        if (inputLayout == InputLayout::BNSD || inputLayout == InputLayout::TND || inputLayout == InputLayout::NTD) {
            OP_CHECK_IF(
                ...
                OPS_REPORT_VECTOR_INNER_ERR(
                    contextKeyParams.opName, // dim num: 3/4
            ...
            OP_CHECK_IF(((keyDim != KV_CACHE_DIM_NUMS_3) && (keyDim != KV_CACHE_DIM_NUMS_5)),
                        OPS_REPORT_VECTOR_INNER_ERR(
                            contextKeyParams.opName,
                            "the layout of query is %s, key and value layout should be [>=%ld, %d, %u] or [>=%ld, %u, "
                            "%u, %d, %d] when PA enable."
                            " now key and value shape [%ld, %ld, %ld, %ld].",
                            layoutStr.c_str(), blockNumValid, *blockSize, queryShapeInfo.h / headNumRatio,
                            blockNumValid, queryShapeInfo.n / headNumRatio,
                            (queryShapeInfo.h / queryShapeInfo.n) * dataTypeSizeValue / BYTE_BLOCK, *blockSize,
                                                     ...
                                                     int64_t headNumRatio, const std::string &sName)
{
    ...
    if (keyDim == 3) {    // dim num: 3
        ...
        OP_CHECK_IF(
            dim3 > HLIMIT,
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "layout of key/value is BSH, the %s h(%ld) should not > 65535 when PA enable",
                                        sName.c_str(), dim3),
            return false);
        ...
        OP_CHECK_IF(((tempN * headNumRatio != shapeInfo.n) || (tempBlockSize != *blockSize) ||
                    ...
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "the shape of %s [%ld, %ld, %ld, %ld] is wrong, which should be "
                                                "[>=%ld, %u, %d, %u] when PA BnNBsD enable!",
                                                sName.c_str(), dim1, dim2, dim3, dim4, blockNumValid,
                                                shapeInfo.n / headNumRatio, *blockSize, (shapeInfo.h / shapeInfo.n)),
                    return false);
    } else {
        ...
        std::vector<ge::DataType> allowedDtypes = {ge::DT_FLOAT16, ge::DT_BF16, ge::DT_INT8, ge::DT_HIFLOAT8,
                                                   ge::DT_FLOAT8_E4M3FN};
        std::vector<uint32_t> dataTypeSizeArray = {FLOAT16SIZE, BFLOAT16SIZE, INT8SIZE,
                                                   FLOAT8SIZE,  FLOAT8SIZE,   FLOAT8SIZE};
        ...
        OP_CHECK_IF(((tempN * headNumRatio != shapeInfo.n) || (tempBlockSize != *blockSize) ||
                    ...
                    OPS_REPORT_VECTOR_INNER_ERR(
                        contextKeyParams.opName,
                        "the shape of %s [%ld, %ld, %ld, %ld, %ld] is wrong, which should be [>=%ld, %u, %u, %d, %d] "
                        "when PA NZ enable!",
                                                        ...
                                                        PromptFlashAttentionTilingDataV2 &tilingData)
{
    ...
    if (!CheckPACacheShape(contextKeyParams, keyDim, queryShapeInfo, keyShape, blockSize, blockNumValid, headNumRatio,
                           "key")) {
        return false;
    }
...
bool PromptFlashAttentionTilingV2::CheckMaskShapeCrossSparse(ContextParamsForPFATiling &contextKeyParams,
                                                             PromptFlashAttentionTilingDataV2 &tilingData,
                                                             const int32_t *sparseMode, uint64_t sQ, const uint64_t sK,
...
bool PromptFlashAttentionTilingV2::CheckIO(ContextParamsForPFATiling &contextKeyParams, PFAShapeInfo &queryShapeInfo,
                                           PFAShapeInfo &valueShapeInfo)
{
    ...
    OP_CHECK_IF(
        ...
        return false);
    OP_CHECK_IF(
        isQKVDDifferent && (outputType != ge::DT_FLOAT16 && outputType != ge::DT_BF16),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
...
bool PromptFlashAttentionTilingV2::CheckKV(ContextParamsForPFATiling &contextKeyParams, PFAShapeInfo &keyShapeInfo,
                                           PFAShapeInfo &valueShapeInfo)
{
    const gert::StorageShape *keyShape = contextKeyParams.keyInputShape;
    const gert::StorageShape *valueShape = contextKeyParams.valueInputShape;
    ...
    OP_CHECK_IF(
        ...
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            "Key type must be float16 or bf16 when query and key headdim is not equal to value headdim."),
        return false);
    OP_CHECK_IF(
        isQKVDDifferent && (contextKeyParams.vDataType != ge::DT_FLOAT16 && contextKeyParams.vDataType != ge::DT_BF16),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            "Value type must be float16 or bf16 when query and key headdim is not equal to value headdim."),
        return false);
                                                    ...
                                                    PromptFlashAttentionTilingDataV2 &tilingData)
{
    ...
    OP_CHECK_IF(!isMaxWorkspace && (queryShapeInfo.b != keyShapeInfo.b) && (!enableTensorList) && (!enablePA),
                OPS_REPORT_VECTOR_INNER_ERR(
                    ...
                    queryShapeInfo.b, keyShapeInfo.b),
                return false);
    // check d size
    OP_CHECK_IF(!isMaxWorkspace && (queryShapeInfo.d != keyShapeInfo.d) && (!enableTensorList) && (!enablePA),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            ...
                                            queryShapeInfo.d, keyShapeInfo.d),
                return false);
    return true;
}

bool PromptFlashAttentionTilingV2::CheckIFAMLA(ContextParamsForPFATiling &contextKeyParams,
                                               const PFAShapeInfo &queryShapeInfo) const
{
    if (enableIFAMLAFullQuant) {
        ...
        OP_CHECK_IF((queryShapeInfo.s < 1),
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                ...
                                                queryShapeInfo.s),
                    return false);
        static const std::set<uint32_t> supportNumHeadInIfaMla = {1U,  2U,  4U,  8U,
                                             ...
                                             PFAShapeInfo &keyShapeInfo, PFAShapeInfo &queryRopeShapeInfo)
{
    ...
    OP_CHECK_IF(
        ...
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "Get and check keyRope shape failed."), return false);
    enableIFA = false;
    enableIFAMask = false;
                                              ...
                                              PFAShapeInfo &keyShapeInfo, const PFAShapeInfo &valueShapeInfo) const
{
    ...
    OP_CHECK_IF(enableKVAntiquant && (enableIFAMLA || enablePFARope || enablePFAMLA),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "MLA does not support antiquant."), return false);
    return true;
}
...
bool PromptFlashAttentionTilingV2::CheckQScaleShape4MLAFullQuant(ContextParamsForPFATiling &contextKeyParams) const
{
    ...
    if (layoutStr == "BSH") {
        OP_CHECK_IF((qDimNum != qScaleDimNum),
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "When MLAFullQuant enables and the layout of query is %s, dim num of "
                                                "dequantScaleQuery(%d) should be equal to queryDimNum(%d).",
                                                layoutStr.c_str(), qScaleDimNum, qDimNum),
                    return false);
        OP_CHECK_IF(
            (queryInputShape.GetDim(0) != dequantScaleQueryShape.GetDim(0)),
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "When MLAFullQuant enables and the layout of query is %s, the %drd dim of "
                                        "dequantScaleQuery(%d) should be equal to queryInputShape(%d).",
                                        layoutStr.c_str(), 0, static_cast<int32_t>(dequantScaleQueryShape.GetDim(0)),
                                        static_cast<int32_t>(queryInputShape.GetDim(0))),
            return false);
        OP_CHECK_IF(
            (queryInputShape.GetDim(1) != dequantScaleQueryShape.GetDim(1)),
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "When MLAFullQuant enables and the layout of query is %s, the %drd dim of "
                                        "dequantScaleQuery(%d) should be equal to queryInputShape(%d).",
            ...
            return false);
        OP_CHECK_IF(
            (*contextKeyParams.headsNumber != dequantScaleQueryShape.GetDim(2)),
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "When MLAFullQuant enables and the layout of query is %s, the %drd dim of "
                                        "dequantScaleQuery(%d) should be equal to numHeads(%d).",
        ...
        OP_CHECK_IF((qScaleDimNum != qDimNum - 1),
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "When MLAFullQuant enables and the layout of query is %s, dim num of "
                                                "dequantScaleQuery(%d) should be equal to [queryDimNum - 1(%d)].",
        ...
        for (uint32_t i = 0; i < qScaleDimNum; i++) {
            OP_CHECK_IF((queryInputShape.GetDim(i) != dequantScaleQueryShape.GetDim(i)),
                        OPS_REPORT_VECTOR_INNER_ERR(
                            contextKeyParams.opName,
                            "When MLAFullQuant enables and the layout of query is %s, the %urd dim of "
                            "dequantScaleQuery(%d) should be equal to queryInputShape(%d).",
...
bool PromptFlashAttentionTilingV2::CheckKVScaleShape4MLAFullQuant(ContextParamsForPFATiling &contextKeyParams) const
{
    ...
    OP_CHECK_IF(
        (keyAntiquantScaleShape->GetStorageShape().GetDimNum() != 1 ||
         valueAntiquantScaleShape->GetStorageShape().GetDimNum() != 1),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "When MLAFullQuant enables, dim num of keyAntiquantScaleShape(%zu) or "
                                    "valueAntiquantScaleShape(%zu) must be 1.",
    ...
    OP_CHECK_IF((keyAntiquantScaleShape->GetStorageShape().GetDim(0) != 1 ||
                ...
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When MLAFullQuant enables, the %drd dim of keyAntiquantScaleShape(%d) or "
                                            "valueAntiquantScaleShape(%d) must be 1.",
                                                     ...
                                                     PFAShapeInfo &queryShapeInfo) const
{
    ...
    OP_CHECK_IF((queryShapeInfo.d != 512),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When MLAFullQuant enables, the d(%d) size of query should be 512.",
                                            queryShapeInfo.d),
                return false);
    // 新增对int8的拦截,关于layout的拦截
    const std::vector<std::string> supportedLayoutListInt8 = {"BSH", "BSND", "TND", "BSH_NBSD", "BSND_NBSD", "TND_NTD"};
    OP_CHECK_IF(
        std::find(supportedLayoutListInt8.begin(), supportedLayoutListInt8.end(), layoutStr) ==
    ...
    OP_CHECK_IF((keyShape->GetStorageShape().GetDimNum() != KV_CACHE_DIM_NUMS_5 &&
                 contextKeyParams.inputDataType == ge::DT_INT8),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "when the dtype of query is int8 in MLA, KV layout must be PANZ."),
                return false);
    ...
    OP_CHECK_IF((inputTypeCheck == allowedDtypes.end() || contextKeyParams.outputDataType != ge::DT_BF16),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When MLAFullQuant enables, dataType of Q(%s), K(%s) and V(%s) must be "
                                            "fp8_e4m3/int8, datatype of output(%s) must be bf16.",
                                            GetPfaDataTypeStr(contextKeyParams.inputDataType).c_str(),
                                            GetPfaDataTypeStr(contextKeyParams.kDataType).c_str(),
                                            GetPfaDataTypeStr(contextKeyParams.vDataType).c_str(),
    ...
    OP_CHECK_IF((*contextKeyParams.keyAntiquantMode != static_cast<int64_t>(AntiquantTypeEnum::PER_CHANNEL) ||
                 *contextKeyParams.valueAntiquantMode != static_cast<int64_t>(AntiquantTypeEnum::PER_CHANNEL)),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When MLAFullQuant enables, keyAntiquantMode (%ld) or valueAntiquantMode "
                                            "(%ld) is InValid! Only support Per-Channel.",
    ...
    OP_CHECK_IF((contextKeyParams.dequantScaleQuery == nullptr),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When MLAFullQuant enables, dequantScaleQuery should not be nullptr."),
                return false);
    ...
    OP_CHECK_IF((contextKeyParams.dequantScaleQueryType != ge::DT_FLOAT ||
                ...
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When MLAFullQuant enables, dataType of dequantScaleQuery(%s), "
                                            "KeyAntiquantScale(%s) and valueAntiquantScale(%s) must be float32.",
                ...
                return false);
    OP_CHECK_IF(
        (!CheckQScaleShape4MLAFullQuant(contextKeyParams) || !CheckKVScaleShape4MLAFullQuant(contextKeyParams)),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "When MLAFullQuant enables, check dequantScaleQuery, "
                                                             "keyAntiQuantScale or valueAntiquantScale shape failed."),
        return false);
                                               ...
                                               PromptFlashAttentionTilingDataV2 &tilingData)
{
    ...
    OP_CHECK_IF(enablePFAMLA,
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "when system prefix is used, query, key and value D should be the same, "
                                            "input query's D and key's D = 192, but value's D = 128!"),
                return false);
...
bool PromptFlashAttentionTilingV2::CheckActSeq(const ContextParamsForPFATiling &contextKeyParams,
                                               const PFAShapeInfo &queryShapeInfo) const
{
    ...
    OP_CHECK_IF(actSeqLen == nullptr,
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When layout is TND/NTD, actualSequenceLengthQ can not be nullptr"),
                return false);
    OP_CHECK_IF(actSeqLenKV == nullptr,
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When layout is TND/NTD, actualSequenceLengthKV can not be nullptr"),
                return false);
    ...
    for (uint32_t i = LOOP_BEGIN_NUM; i < batchSize; ++i) {
        ...
        OP_CHECK_IF(curActSeq < 0,
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "actualSeqLengths[%u] = %ld, should >= 0", i,
                                                curActSeq),
                    return false);
        OP_CHECK_IF(((i >= 1) && (curActSeq < lastActSeq)),
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "When layout is TND, actualSeqLengths must not be decreasing, but "
                                                "actSeqLen[%u]=%ld is smaller than actSeqLen[%u]=%ld",
                                                i, curActSeq, i - 1, lastActSeq),
                    return false);
        OP_CHECK_IF(curActSeqKV < 0,
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "actualSeqLengthsKv[%u] = %ld, should >= 0", i,
                                                curActSeqKV),
                    return false);
        if (!enablePA) {
            OP_CHECK_IF(((i >= 1) && (curActSeqKV < lastActSeqKV)),
                        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                    "When layout is TND, actualSeqLengthsKv must not be decreasing, "
                                                    "but actSeqLenKV[%u]=%ld is smaller than actSeqLenKV[%u]=%ld",
                                                  ...
                                                  PFAShapeInfo &queryShapeInfo, PFAShapeInfo &keyShapeInfo)
{
    ...
    if (enableActSeqLen) { // check the length of actual_seq_lengthsQ, whether is 1 or batch size
        OP_CHECK_IF(
            enableIFAMLAFullQuant && (inputLayout != InputLayout::TND),
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "The layout is %s, Actual_seq_lengths cannot be configured in MLA Fullquant "
                                        "and non-TND/TND_NTD scenarios, only supported when layout is TND/TND_NTD!",
                                        layoutStr.c_str()),
            return false);
        ...
        for (uint32_t i = LOOP_BEGIN_NUM; i < actSeqLengthKVSize; ++i) {
            ...
            if (!enableTensorList && !enablePA) {
                ...
                OP_CHECK_IF((actSeqKVTmp < 0) || (actSeqKVTmp > actSeqKVMax),
                            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                        "Actual_seq_lengths_kv[%u](%ld) must be in range[0, %ld]!", i,
                                                        actSeqKVTmp, actSeqKVMax),
                            return false);
                                                       ...
                                                       PromptFlashAttentionTilingDataV2 &tilingData)
{
    OP_CHECK_IF(isQKVDDifferent,
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "Not support PA when query and key headdim is not equal to value headdim."),
                return false);
    ...
    OP_CHECK_IF(blockTableShape->GetStorageShape().GetShapeSize() == 0,
                OPS_REPORT_VECTOR_INNER_ERR(
                    contextKeyParams.opName, "blockTable each dim can't be 0, blockTable Shape [%ld, %ld].",
    ...
    if (enablePertensorQuant) {
        OP_CHECK_IF(
            ...
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "block size(%d) should be a multiple of %d, and should be in range of [%d, %d] "
                                        "when Full Quant and PA enable",
        ...
        OP_CHECK_IF((!enableIFAMLA && !enableIFA && !(queryShapeInfo.s == 1 && enableAlibiPse) &&
                    ...
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "block size(%d) should be a multiple of %d, and should be in range of "
                                                "[%d, %d] when PA enable and no quant",
    ...
    OP_CHECK_IF((enableIFAMLAFullQuant && *blockSize != BLOCK_SIZE_BASE),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "block size(%d) should be %d, when MLA Full Quant and PA enable",
                                            *blockSize, BLOCK_SIZE_BASE),
                return false);
    // 非量化场景
    if (!enableIFAMLAFullQuant && !enablePertensorQuant) {
        if (enableIFAMLA || enablePFARope || enablePFAMLA) {
            OP_CHECK_IF((*blockSize % BLOCK_SIZE_BASE_FOR_NO_QUANT != 0 || *blockSize < BLOCK_SIZE_BASE_FOR_NO_QUANT ||
                         *blockSize > BLOCK_SIZE_MAX_FOR_NO_QUANT),
                        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                    "block size(%d) should be a multiple of %d, and should be in range "
                                                    "of [%d, %d] when PA enable and no quant mla",
                                                    *blockSize, BLOCK_SIZE_BASE_FOR_NO_QUANT,
                                                    BLOCK_SIZE_BASE_FOR_NO_QUANT, BLOCK_SIZE_MAX_FOR_NO_QUANT),
                        return false);
            ...
            OP_CHECK_IF(
                 ...
                 *blockSize > BLOCK_SIZE_MAX_FOR_NO_QUANT),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "block size(%d) should be a multiple of %d, and should be in range of [%d, "
                                            "%d] when PA enable and no quant with d[%d] is 64/128",
                                            *blockSize, BLOCK_SIZE_BASE_FOR_NO_QUANT, BLOCK_SIZE_BASE_FOR_NO_QUANT,
                                            BLOCK_SIZE_MAX_FOR_NO_QUANT, queryShapeInfo.d),
                return false);
        } else {
            if (queryShapeInfo.s > 1) {
                OP_CHECK_IF(
                    ...
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "block size(%d) should be a multiple of %d, and should be in range of "
                                                "[%d, %d] when PA enable and no quant with qs[%d] is greater than 1",
                                                *blockSize, BLOCK_SIZE_BASE, BLOCK_SIZE_BASE, BLOCK_SIZE_MAX,
                                                queryShapeInfo.s),
                    return false);
            } else {
                OP_CHECK_IF(
                    ...
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "block size(%d) should be a multiple of %d, and should be in range of "
                                                "[%d, %d] when PA enable and no quant with qs[%d] is 1",
                                                *blockSize, BLOCK_SIZE_BASE_FOR_NO_QUANT, BLOCK_SIZE_BASE_FOR_NO_QUANT,
                                                BLOCK_SIZE_MAX, queryShapeInfo.s),
                    return false);
            }
        }
    }
    ...
    if (!CheckBlockTableShape(contextKeyParams, queryShapeInfo, queryRopeShapeInfo, blockSize, blockTableShape,
                              tilingData)) {
        return false;
    }
...
bool PromptFlashAttentionTilingV2::CheckPseShiftTypeAndShape(ContextParamsForPFATiling &contextKeyParams, uint32_t b,
                                                             uint32_t n, uint32_t s1, uint32_t s2)
{
    ...
    OP_CHECK_IF(
        isQKVDDifferent,
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "Not support pse shift when query and key headdim is not equal to value headdim."),
        return false);
...
bool PromptFlashAttentionTilingV2::CheckSparseMode(ContextParamsForPFATiling &contextKeyParams, uint32_t qS)
{
    ...
    SetSparseType(qS);
    OP_CHECK_IF(
        (isQKVDDifferent && sparseModeVal != 0 && sparseModeVal != 2 && sparseModeVal != 3),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
    ...
    if (enableActSeqLenKV && !enableTensorList) {
        ...
        for (uint32_t i = LOOP_BEGIN_NUM; i < actSeqLenKVSize; ++i) {
            OP_CHECK_IF(enablePA && (actSeqLenKV->GetData<int64_t>()[i] < 0),
                        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "Actual_seq_lengths_kv[%u](%ld) must >= 0",
                                                    i, actSeqLenKV->GetData<int64_t>()[i]),
                        return false);
        }
    }
    return true;
    ...
    if (enableIFAMLAFullQuant) {
        if (queryS == 1U) {
            ...
            if (contextKeyParams.inputDataType == ge::DT_FLOAT8_E4M3FN) {
                OP_CHECK_IF(
                    !(((*sparseMode == SPARSE_MODE_RIGHT_DOWN) && (enableMask)) ||
                      ((*sparseMode == SPARSE_MODE_NO_MASK) && (!enableMask))),
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "Only support sparse 3 with mask, or sparse 0 without mask when ifa "
                                                "mla fullquant fp8 and query's sequence length is > 1, "
                                                "input sparse mode is %d and there has%smask",
                                                *sparseMode, enableMask ? " " : " no "),
                    return false);
                ...
                OP_CHECK_IF(!((*sparseMode == SPARSE_MODE_RIGHT_DOWN) && (enableMask)),
                            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                        "Only support sparse 3 with mask, when ifa mla fullquant int8 "
                                                        "and query's sequence length is > 1, "
                                                        "input sparse mode is %d and there has%smask",
                                                        *sparseMode, enableMask ? " " : " no "),
                            return false);
            }
                                                      ...
                                                      PromptFlashAttentionTilingDataV2 &tilingData)
{
    ...
    OP_CHECK_IF((contextKeyParams.attentionMask != nullptr && maskDataType == ge::DT_FLOAT16 &&
                ...
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            ...
                                            GetPfaDataTypeStr(maskDataType).c_str()),
                return false);
    if (!CheckMaskShapeCrossSparse(contextKeyParams, tilingData, sparseMode, queryShapeInfo.s,
                                   S2 + actualSharedPrefixLen, queryShapeInfo.b)) {
        return false;
    }
                                                           ...
                                                           PFAShapeInfo &queryShapeInfo)
{
    ...
    OP_CHECK_IF(enableLeftPadding,
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When layout is %s, left padding is not supported!", layoutStr.c_str()),
                return false);
                                                                 ...
                                                                 PFAShapeInfo &queryShapeInfo)
{
    ...
    if (layoutStr == "BSH_BNSD" || layoutStr == "BSND_BNSD") {
        ...
        OP_CHECK_IF(enableTensorList,
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "When layout is %s, tensorlist is not supported!", layoutStr.c_str()),
                    return false);
        ...
        if (isGqa) { // GQA
            OP_CHECK_IF((contextKeyParams.outputDataType == ge::DT_INT8 && queryShapeInfo.d % D_SIZE_BASE_32 != 0),
                        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                    "In GQA scenario, when layout is %s and output dtype is int8, d "
                                                    "size should be a multiple of %d, but got d = %d.",
                                                  ...
                                                  PFAShapeInfo &queryShapeInfo)
{
    ...
    OP_CHECK_IF(contextKeyParams.learnableSinkDataType != contextKeyParams.inputDataType,
                ...
                return false);
    OP_CHECK_IF(
        contextKeyParams.learnableSinkDataType != ge::DT_BF16 &&
        ...
        return false);
    OP_CHECK_IF(
        queryShapeInfo.d != 192 && queryShapeInfo.d != 128 && queryShapeInfo.d != 64,
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            "When learnable sink is used, query headdim must be one of {192, 128, 64}, but got %u.", queryShapeInfo.d),
        return false);
    OP_CHECK_IF(
        enablePseShift,
    ...
    OP_CHECK_IF(
        ...
        return false);
    OP_CHECK_IF(
        enableIFAMLAFullQuant || enableIFAMLA || enablePerblockQuant || enablePertensorQuant,
    ...
    for (uint32_t i = LOOP_BEGIN_NUM; i < lenDims; i++) {
        ...
        if (enableAlibiPse) {
            OP_CHECK_IF((actualSeqLengths[i] != actualSeqLengthsKV[i]),
                        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                    "When pseType = 2/3, actualSeqLengths[%u](seq size of query)=%ld "
                                                    "must be equal to actualSeqLengthsKv[%u](seq size of key)=%ld",
                                                          ...
                                                          PromptFlashAttentionTilingDataV2 &tilingData)
{
    ...
    OP_CHECK_IF(
        enableActSeqLen && (inputLayout == InputLayout::TND),
    ...
    OP_CHECK_IF(tilingData.promptAttentionBaseParams.get_isRowInvalid() != 0,
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "innerPrecise(%u) must be 0 or 1 in per-block quant scenario!",
                                            tilingData.promptAttentionBaseParams.get_isRowInvalid()),
                return false);
    OP_CHECK_IF(
...
void PromptFlashAttentionTilingV2::InferTilingMod(const ContextParamsForPFATiling &contextKeyParams,
                                                  std::vector<int64_t> &actualSeqLengths,
                                                  std::vector<int64_t> &actualSeqLengthsKV, uint32_t actualSeqArrayLen,
                                                  uint32_t d)
{
                                                                 ...
                                                                 uint32_t sInnerFactor, bool autoBaseMNK)
{
    ...
    OP_CHECK_IF(ret != 0,
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName,
                                            "bmm1 SetFixSplit failed, l1SizeRemain = %ld, l0CSize = %ld, sOuterFactor "
                                            "= %u, sInnerFactor = %u, ret = %d!",
    ...
    OP_CHECK_IF(ret != 0,
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName,
                                            ...
                                            "%u, sInnerFactor = %u, autoBaseMNK = %d,"
                                            "ret = %d!",
...
bool PromptFlashAttentionTilingV2::PromptFlashAttentionComputeCVDiffParams(PromptFlashAttentionTilingDataV2 &tilingData,
                                                                           int64_t l1Size, int64_t l0CSize,
                                                                           uint32_t &sOuterFactor,
                                                                           uint32_t &sInnerFactor)
{
    constexpr uint32_t dSplitFactorBmm2 = 128U;
    int32_t l1SizeRemain = l1Size;
    bool res = PromptFlashAttentionCheckBmm1(tilingData, tilingData.bmm1TilingDataRect, l1SizeRemain, l0CSize,
                                             sOuterFactor, sInnerFactor, true);
...
int64_t PromptFlashAttentionTilingV2::GetCutBlockNums(int64_t blockSeqLengthKV, int64_t blockSeqLength, int64_t sInner,
                                                      int64_t sOuter, int64_t token) const
{
                                                              ...
                                                              bool isAttenMaskUsed) const
{
    if (!isAttenMaskUsed) {
        ...
        toCalcBlockNums -=
            GetCutBlockNums(blockSharedPrefix, blockSeqLength, static_cast<int64_t>(sInnerSize),
                                                     ...
                                                     uint32_t sInnerSize, double coreWightTarget, uint32_t &curCore)
{
    ...
    for (uint32_t sIdx = 0; sIdx < batchSize; sIdx++) {
        for (uint32_t headNum = 0; headNum < baseParams->get_headNumSize(); headNum++) {
            ...
            FixParamWithRowInvalid(actualSeqLength, actualSeqLengthKV + actualSharedPrefixLen, preTokensLeftUp,
                                   nextTokensLeftUp);
                                                                  ...
                                                                  bool isAttenMaskUsed)
{
    ...
    for (uint32_t sIdx = 0; sIdx < batchSize; sIdx++) {
        ...
        FixParamWithRowInvalid(actualSeqLengthsTmp, actualSeqLengthsKV[sIdx] + actualSharedPrefixLen, preTokensLeftUp,
                               nextTokensLeftUp);
        ...
        totalBlockNumsOneHead +=
            GetCalcBlockNumsOneHead(actualSeqLengthsTmp, actualSeqLengthsKV[sIdx], sOuterSize, sInnerSize,
    ...
    SetMultiCoreParamsRegbase((totalBlockNumsOneHead / sinnerBlocknum) * baseParams->get_headNumSize(),
                              static_cast<int64_t>((curIndx + 1)));
}
...
void PromptFlashAttentionTilingV2::UpdateTilingKeyConfig(ContextParamsForPFATiling &contextKeyParams,
                                                         PromptFlashAttentionTilingDataV2 &tilingData)
{
...
void PromptFlashAttentionTilingV2::UpdateTilingKeyPFAMask(PromptFlashAttentionTilingDataV2 &tilingData,
                                                          ge::DataType inputDataType)
{
    if (enablePerblockQuant || enableIFAMLAFullQuant || inputDataType == ge::DT_FLOAT16 ||
        inputDataType == ge::DT_BF16) {
        PFAMask = 0;
        return;
    }
...
void PromptFlashAttentionTilingV2::UpdateTilingKeyPFAMatMulType(PromptFlashAttentionTilingDataV2 &tilingData,
                                                                ge::DataType inputDataType)
{
    if (enablePerblockQuant || enableIFAMLAFullQuant || inputDataType == ge::DT_FLOAT16 ||
        inputDataType == ge::DT_BF16) {
...
bool PromptFlashAttentionTilingV2::TilingGetTilingKeyAttentionAscendC(ContextParamsForPFATiling &contextKeyParams,
                                                                      PromptFlashAttentionTilingDataV2 &tilingData)
{
...
size_t PromptFlashAttentionTilingV2::GetPFAWorkSpaceSize(PromptFlashAttentionTilingDataV2 &tilingData)
{
    ...
    // 2:use 2mm ub
    if (!faRunFlag_) {
        curWorkspaceSize = sysWorkspaceSize + coreNum * softmaxDataTypeSize *
                                                  (maxSpmSize + mm1ResSize * MM2_UB_NUM + mm2ResSize * MM2_UB_NUM);
    } else {
        ...
        if (isMaxWorkspace) { // 计算maxWorkSpaceSize时默认开启FD且使用最大核数进行归约
            ...
            logSumExpSize = aicNum * BYTE_BLOCK * 2; // 2: Each AI core needs 2 byte blocks for storing log-sum-exp
                                                     // intermediate results (fixed multiplier factor)
...
ge::graphStatus PromptFlashAttentionTilingV2::SetAttributeInfo(ContextParamsForPFATiling &contextKeyParams)
{
    ...
    OP_CHECK_IF(!VecContains(inOutDtypeSupported, inOutDtypeTuple),
                OPS_REPORT_VECTOR_INNER_ERR(
                    contextKeyParams.opName,
                    "Query dtype(%s), key/value dtype(%s), attentionOut dtype(%s) is not currently supported.",
                    GetPfaDataTypeStr(contextKeyParams.inputDataType).c_str(),
                    GetPfaDataTypeStr(contextKeyParams.kDataType).c_str(),
                    GetPfaDataTypeStr(contextKeyParams.outputDataType).c_str()),
                return ge::GRAPH_FAILED);
    ...
    if ((contextKeyParams.queryRopeInputShape != nullptr && contextKeyParams.keyRopeInputShape != nullptr) &&
        ((contextKeyParams.inputDataType == ge::DT_FLOAT8_E4M3FN &&
          contextKeyParams.kDataType == ge::DT_FLOAT8_E4M3FN && contextKeyParams.vDataType == ge::DT_FLOAT8_E4M3FN) ||
...
bool PromptFlashAttentionTilingV2::CheckAlibiPseShiftTypeAndShape(ContextParamsForPFATiling &contextKeyParams,
                                                                  uint32_t n)
{
    ...
    OP_CHECK_IF(
        isQKVDDifferent,
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "Not support alibi pse when query and key headdim is not equal to value headdim."),
        return false);
...
ge::graphStatus PromptFlashAttentionTilingV2::CheckSingleAttribute(
    ContextParamsForPFATiling &contextKeyParams, PFAShapeInfo &queryShapeInfo, PFAShapeInfo &keyShapeInfo,
    PFAShapeInfo &valueShapeInfo, PFAShapeInfo &queryRopeShapeInfo, PromptFlashAttentionTilingDataV2 &tilingData)
{
    if (!CheckIO(contextKeyParams, queryShapeInfo, valueShapeInfo)) {
        OP_LOGE(contextKeyParams.opName, "Check query/output failed!");
        return ge::GRAPH_FAILED;
    }
    ...
    // pse check
    if (enablePseShift) {
        if (!CheckPseShiftTypeAndShape(contextKeyParams, queryShapeInfo.b, queryShapeInfo.n, queryShapeInfo.s,
                                       S2 + actualSharedPrefixLen)) {
            return ge::GRAPH_FAILED;
        }
        usePseShift = 1;
...
bool PromptFlashAttentionTilingV2::CheckAlibiPseCrossover(ContextParamsForPFATiling &contextKeyParams)
{
    ...
    OP_CHECK_IF((enableIFAMLA || enablePFARope),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When pseType = 2/3, rope is not supported, pseType = %ld", pseType),
                return false);

    OP_CHECK_IF((enablePFAMLA),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                            "When pseType = 2/3, query, key and value D should be the same, input "
                                            "query's D and key's D = 192, but value's D = 128."),
                return false);
                                                               ...
                                                               const PFAShapeInfo &queryShapeInfo)
{
    ...
    auto ret = AdjustCVTilingCVDiff(contextKeyParams, sOuterFactor, sInnerFactor, softmaxSOuterFactor, tilingData,
                                    queryShapeInfo);
...
bool PromptFlashAttentionTilingV2::IsFlashDecode(ContextParamsForPFATiling &contextKeyParams, uint64_t bng) const
{
    ...
    if ((bng < flashDecodeBNRatio * aicNum) && (gSize == 1)) {
        OP_LOGD(contextKeyParams.opName, "Flash decode split key/value.");
        return true;
    }
...
ge::graphStatus PromptFlashAttentionTilingV2::SplitBNS(PromptFlashAttentionTilingDataV2 &tilingData, uint64_t bng)
{
    ...
    faTilingAdapter.inputParamsRegbase.set_logSumExpSize(batchSize * headNumSize * kvSplitPart *
...
void PromptFlashAttentionTilingV2::SetLayoutType()
{
    static std::map<InputLayout, LayoutType> layoutStrToLayoutTypeMap = {
        {InputLayout::BSH, LayoutType::LAYOUT_BSH},   {InputLayout::TND, LayoutType::LAYOUT_TND},
        {InputLayout::BSND, LayoutType::LAYOUT_BSND}, {InputLayout::BNSD, LayoutType::LAYOUT_BNSD},
...
ge::graphStatus PromptFlashAttentionTilingV2::SetQKVStartIdx(ContextParamsForPFATiling &contextKeyParams)
{
            ...
            if (kvValue != nullptr) {
                ...
                OP_CHECK_IF(kvStartIdx > INT32_MAX || kvStartIdx < INT32_MIN,
                            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                        ...
                                                        INT32_MIN, INT32_MAX, kvStartIdx),
                            return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(kvStartIdx - qStartIdx > POS_SHIFT_MAX || kvStartIdx - qStartIdx < POS_SHIFT_MIN,
                OPS_REPORT_VECTOR_INNER_ERR(
                    ...
                    POS_SHIFT_MIN, POS_SHIFT_MAX, qStartIdx, kvStartIdx),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
    ...
    ContextParamsForPFATiling &contextKeyParams) const
{
    ...
    bool inputOutputIsNullPtr =
        (context_->GetInputDesc(QUERY_INDEX) == nullptr) || (context_->GetInputDesc(KEY_INDEX) == nullptr) ||
                                                                           ...
                                                                           PromptFlashAttentionTilingDataV2 &tilingData)
{
    ...
    if (enablePerblockQuant) {
        ...
        if ((inputLayout == InputLayout::BNSD && contextKeyParams.valueAntiquantScaleShape != nullptr &&
             contextKeyParams.valueAntiquantScaleShape->GetStorageShape().GetDim(2) ==
                 CeilDivision(valueShapeInfo.s, static_cast<uint64_t>(optFp8VBlockSize)))) {
    ...
    if (isMaxWorkspace) {
        ...
        if (!CheckMultiFeatureCrossover(contextKeyParams, queryShapeInfo, actualSeqLengths, actualSeqLengthsKV,
                                        tilingData)) {
            return ge::GRAPH_FAILED;
        }
    }
...
void PromptFlashAttentionTilingV2::SetTilingKey(ContextParamsForPFATiling &contextKeyParams) const
{
    uint64_t gen_tilingkey = GET_TPL_TILING_KEY(
        ...
        static_cast<uint64_t>(enableS1OutSplit));
...
ge::graphStatus PromptFlashAttentionTilingV2::DoSubOpTiling(PromptFlashAttentionTilingDataV2 &tilingData,
                                                            ContextParamsForPFATiling &contextParamsForPFATiling)
{
    ...
    OP_CHECK_IF(ret == ge::GRAPH_FAILED,
                OPS_REPORT_VECTOR_INNER_ERR(context_->GetNodeName(), "fail to parse tiling params!"),
                return ge::GRAPH_FAILED);
    ...
    if (faRunFlag_) {
        ...
        *tiling = faTilingAdapter;
        ...
        uint64_t cap = context_->GetRawTilingData()->GetCapacity();
        OP_LOGI(contextParamsForPFATiling.opName, "TilingData context GetCapacity: %lu, faRunFlag_ is %d", cap,
                faRunFlag_);
        if (contextParamsForPFATiling.inputDataType != ge::DT_BF16 &&
            contextParamsForPFATiling.inputDataType != ge::DT_FLOAT16) {
