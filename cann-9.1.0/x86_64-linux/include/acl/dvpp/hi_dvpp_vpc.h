...
#define VPC_ENGINE_MAX_PIC_NUM 8
...
typedef struct{
    ...
    void *lut; // 可支持小型化和非小型化，内存由用户提供，要求128byte向上对齐
    hi_u32 mask_len;
    hi_u8 *mask;
    hi_u32 blend_table_len;
    hi_u8 *blend_table;
...
typedef enum {
    HI_STITCHING_TABLE_IPM,
    HI_STITCHING_TABLE_LUT_TYPE1,
    HI_STITCHING_TABLE_LUT_TYPE2,
...
typedef enum {
    HI_MINI_ORDER_1 = 0, // 非小型化
    HI_MINI_ORDER_16 = 1, // 1/16倍小型化变换表
    HI_MINI_ORDER_4 = 2, // 1/4倍小型化变换表
} hi_mini_order;
...
typedef struct {
    ...
    hi_u32 count;
    hi_vpc_pic_info *source_pic;
    hi_chassis_pic_param chassis_pic;
 ...
 * @param [in] chn: vpc channel id [0, VPC_MAX_CHN_NUM)
 * @param [in] blend_param: blend parameter
 * @param [in] milli_sec: -1 is block,0 is no block,other positive number is timeout
 * @param [out] task_id: pointer of the task ID, used to distinguish tasks.
 * @return success: return 0
 *         fail: return error number
 */
hi_s32 hi_mpi_vpc_blend(hi_vpc_chn chn, hi_blend_param* blend_param, hi_u32* task_id, hi_s32 milli_sec);

/*
 * @brief : convert raw to rgb and support resize
 * @param [in] chn: vpc channel id [0, VPC_MAX_CHN_NUM)
 * @param [in] hi_csc_param: raw csc convert parameter
 * @param [in] milli_sec: -1 is block,0 is no block,other positive number is timeout
 * @param [out] task_id: pointer of the task ID, used to distinguish tasks.
 * @return success: return 0
 *         fail: return error number
 */
hi_s32 hi_mpi_vpc_csc(hi_vpc_chn chn, hi_csc_param *csc_param, hi_u32 *task_id, hi_s32 milli_sec);

/*
 * @brief : create a vpc stars engine config handle
 * @param [in] handle_type: handle type
 * @param [out] handle: the created handle
 * @param [in] handle_param: the handle param
 * @param [in] param_size: the size of the handle param
 * @return success: return 0
 *         fail: return error number
 */
hi_s32 hi_mpi_vpc_create_engine_config_handle(hi_vpc_handle handle_type, hi_s32 *handle,
    const void *handle_param, const hi_u32 param_size);

/*
 * @brief : destroy a vpc stars engine config handle
 * @param [in] handle: the handle to be destroyed
 * @return success: return 0
 *         fail: return error number
 */
hi_s32 hi_mpi_vpc_destroy_engine_config_handle(hi_s32 handle);

/*
 * @brief : update vpc stars engine config handle
 * @param [in] handle: the handle to be update
 * @param [in] handle_param: the handle param
 * @param [in] param_size: the size of the handle param
 * @return success: return 0
 *         fail: return error number
 */
hi_s32 hi_mpi_vpc_set_engine_config_handle(hi_s32 handle, const void *handle_param, const hi_u32 param_size);
