 ...
 * @param [in] data:
 * 表示参与计算的目标张量，维度小于8维，Device侧的aclTensor，支持[非连续的Tensor](../../../../docs/zh/context/非连续的Tensor.md)，
 ...
 * @param [in] noopWithEmptyAxes:
 * 指定axes为空时的行为：false即对所有轴进行计算；true即不进行计算，输出张量等于输入张量，Host侧的BOOL值。
 * @param [in] reduce:
 * 表示计算后的结果，维度小于8维，Device侧的aclTensor，支持[非连续的Tensor](../../../../docs/zh/context/非连续的Tensor.md)，
 * 数据类型支持FLOAT16、FLOAT32，需与data一致，[数据格式](../../../../docs/zh/context/数据格式.md)支持ND。
 * @param [out] workspaceSize: 返回用户需要在npu device侧申请的workspace大小。
 * @param [out] executor: 返回op执行器，包含算子计算流程。
 * @return aclnnStatus: 返回状态码。
 */
ACLNN_API aclnnStatus aclnnReduceLogSumGetWorkspaceSize(
...
aclnnReduceLogSum(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream);
