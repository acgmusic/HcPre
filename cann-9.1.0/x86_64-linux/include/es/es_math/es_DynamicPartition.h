...
#ifndef AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_DynamicPartition_CPP_H_
#define AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_DynamicPartition_CPP_H_
#include <utility>
 ...
 */
inline std::vector<EsTensorHolder> DynamicPartition(const EsTensorLike & x, const EsTensorLike & partitions, int64_t y_num, int64_t num_partitions=1) {
  auto *owner_graph_builder = ResolveBuilder(x, partitions);
  ...
  auto out = EsDynamicPartition(x.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), partitions.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), y_num, num_partitions);
