...
#ifndef AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_SplitV_CPP_H_
#define AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_SplitV_CPP_H_
#include <utility>
 ...
 */
inline std::vector<EsTensorHolder> SplitV(const EsTensorLike & x, const EsTensorLike & size_splits, const EsTensorLike & split_dim, int64_t y_num, int64_t num_split) {
  auto *owner_graph_builder = ResolveBuilder(x, size_splits, split_dim);
  ...
  auto out = EsSplitV(x.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), size_splits.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), split_dim.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), y_num, num_split);
