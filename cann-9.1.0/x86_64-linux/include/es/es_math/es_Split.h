...
#ifndef AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_Split_CPP_H_
#define AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_Split_CPP_H_
#include <utility>
 ...
 */
inline std::vector<EsTensorHolder> Split(const EsTensorLike & split_dim, const EsTensorLike & x, int64_t y_num, int64_t num_split) {
  auto *owner_graph_builder = ResolveBuilder(split_dim, x);
  ...
  auto out = EsSplit(split_dim.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), x.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), y_num, num_split);
