...
#ifndef AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_ForeachMinimumScalarList_CPP_H_
#define AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_ForeachMinimumScalarList_CPP_H_
#include <utility>
 ...
 */
inline std::vector<EsTensorHolder> ForeachMinimumScalarList(const std::vector<EsTensorHolder> & x, const EsTensorLike & scalars, int64_t y_num) {
  auto *owner_graph_builder = ResolveBuilder(x, scalars);
  ...
  auto out = EsForeachMinimumScalarList(esb_x.data(), static_cast<int64_t>(esb_x.size()), scalars.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), y_num);
