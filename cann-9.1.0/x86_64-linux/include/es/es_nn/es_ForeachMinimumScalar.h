...
#ifndef AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_ForeachMinimumScalar_CPP_H_
#define AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_ForeachMinimumScalar_CPP_H_
#include <utility>
 ...
 */
inline std::vector<EsTensorHolder> ForeachMinimumScalar(const std::vector<EsTensorHolder> & x, const EsTensorLike & scalar, int64_t y_num) {
  auto *owner_graph_builder = ResolveBuilder(x, scalar);
  ...
  auto out = EsForeachMinimumScalar(esb_x.data(), static_cast<int64_t>(esb_x.size()), scalar.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), y_num);
