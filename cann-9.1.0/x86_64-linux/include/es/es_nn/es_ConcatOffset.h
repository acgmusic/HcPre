...
#ifndef AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_ConcatOffset_CPP_H_
#define AIR_CXX_COMPILER_GRAPH_EAGER_STYLE_GRAPH_BUILDER_OP_ConcatOffset_CPP_H_
#include <utility>
 ...
 */
inline std::vector<EsTensorHolder> ConcatOffset(const EsTensorLike & concat_dim, const std::vector<EsTensorHolder> & x, int64_t y_num, int64_t N) {
  auto *owner_graph_builder = ResolveBuilder(concat_dim, x);
  ...
  auto out = EsConcatOffset(concat_dim.ToTensorHolder(owner_graph_builder).GetCTensorHolder(), esb_x.data(), static_cast<int64_t>(esb_x.size()), y_num, N);
